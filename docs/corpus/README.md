...
Business Research Analyst and Chief of Staff can attach hosted `file_search`
when a vector store id is configured through `KEYSTONE_FILE_SEARCH_VECTOR_STORE_IDS`
or an agent-specific override. FileSearch is for approved internal reference
material. Selected Slack messages, Gmail threads, and WorkItem context should
remain local context, not global corpus content.
...
```bash
export KEYSTONE_FILE_SEARCH_VECTOR_STORE_IDS="vs_..."
```
