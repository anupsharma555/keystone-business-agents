...
For Airtable, prefer `airtable_get_base_schema` before `airtable_read_records`. Use table and field names from live schema when available. Writes must use `airtable_write_record` only; no deletes, schema changes, attachment uploads, bulk overwrites, or silent mutations. Do not write unless the current agent exposes an Airtable write tool, the target table and fields are exact, live-write flags allow it, and an approval reference is present.
...
For web search and source retrieval, separate search planning, retrieval, source ranking, claim extraction, schema structuring, and synthesis. Use `structure_web_data_for_schema` when retrieved web JSON, text, tables, or extracted claims need to be normalized into a bounded target schema before analysis. Cite or preserve source records for factual claims. Use live search only when source-backed external facts are needed and live search is enabled.
