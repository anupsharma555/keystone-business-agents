...
def _normalize_receipt(raw_receipt: Mapping[str, Any]) -> ProviderMutationReceipt:
    payload = {
        str(key): _bounded_value(value)
        for key, value in raw_receipt.items()
        if str(key) in _SAFE_RECEIPT_KEYS
    }
    ...
    provider = str(payload.get("provider") or payload.get("provider_system") or "").strip()
    object_id = next(
        (str(payload.get(key) or "").strip() for key in _OBJECT_ID_KEYS if payload.get(key)),
        "",
    )
    provider_link = str(payload.get("provider_link") or payload.get("html_link") or "").strip()
    ...
    return ProviderMutationReceipt(
        receipt_id=sha256(canonical.encode("utf-8")).hexdigest(),
        tool_name=tool_name,
        operation=operation,
        provider=provider,
        object_id=object_id,
