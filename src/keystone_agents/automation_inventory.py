...
def automation_spec_for_command(command: str, *, stage: str = "") -> AutomationSpec:
    ...
    metadata = json.dumps({"last_stage": stage}, ensure_ascii=True, sort_keys=True)
    if normalized == "weekly-opportunity":
        return DEFAULT_AUTOMATION_SPECS[0].model_copy(update={"metadata": metadata})
    if normalized == "gmail-triage":
        return DEFAULT_AUTOMATION_SPECS[1].model_copy(update={"metadata": metadata})
    if normalized == "chief-of-staff-weekly-meeting-prep":
        return DEFAULT_AUTOMATION_SPECS[3].model_copy(update={"metadata": metadata})
    if normalized in {"announcements-weekly-research-synthesis", "announcements-research"}:
        return DEFAULT_AUTOMATION_SPECS[4].model_copy(update={"metadata": metadata})
    if normalized == "github-repo-opportunities-weekly":
        return DEFAULT_AUTOMATION_SPECS[5].model_copy(update={"metadata": metadata})
