    ...
    try:
        ...
        if failure is None:
            ...
            if failed_snapshot.first_feedback_ms is not None:
                scope.recorder.mark_final_response()
        telemetry = compact_execution_telemetry(scope.recorder.snapshot(status=telemetry_status))
...
def _persist_entry_execution_telemetry(
    ...
    if not telemetry:
        return
    try:
        store = SQLiteStore(scope.database_url or database_url_from_env())
